...
def prepare_embeddings(saved_embeddings):
    ...
    with open(saved_embeddings, 'wb') as f:
        pickle.dump((frequencies, word_embedding_map), f)
...
def load_embeddings(saved_embeddings):
    ...
    """
    (frequencies, word_embedding_map) = pickle.load(bz2.BZ2File(saved_embeddings, 'r'), encoding='bytes')
...
def dictionary_and_embeddings_to_indices(word_frequencies, embeddings):
    ...
    index_to_word_map = np.empty(len(word_frequencies.keys()) + offset, dtype=object)    
    ...
    for idx, word in enumerate(sorted_word_frequencies_keys):
        ...
        index_to_word_map[new_index] = word
    ...
    return word_to_indices_map, word_index_to_embeddings_map, index_to_word_map
...
def load_all(serialized_file='vocabulary.embeddings.all.pkl.bz2'):
    ...
    word_to_indices_map, word_index_to_embeddings_map, index_to_word_map = dictionary_and_embeddings_to_indices(freq, embeddings_map)
